...
namespace Squared.Tiled
{
    ...
    public class Layer : IDrawable
    {
        ...
        public void Draw(Map result, SpriteBatch batch, IList<Tileset> tilesets, Rectangle rectangle, Vector2 viewportPosition, int tileWidth, int tileHeight)
        {
            int i = 0;
    ...
    public class Map
    {
        ...
        public static Map Load(string filename, ContentManager content)
        {
                                ...
                                case "layer":
                                    {
                                        using (var st = reader.ReadSubtree())
                                        {
                                            ...
                                            if (null != layer)
                                            {
                                                result.Drawables.Add(layer.Name, layer);
            ...
            {
                var st2 = Path.GetDirectoryName(filename).Replace(content.RootDirectory, "");
                var st3 = st2.Trim('\\', '/');
                var st = Path.Combine(Path.GetDirectoryName(tileset.Image), Path.GetFileNameWithoutExtension(tileset.Image));
        ...
        public void Draw(SpriteBatch batch, Rectangle rectangle, Vector2 viewportPosition)
        {
            ...
            {
                drawable.Draw(this, batch, new List<Tileset>(Tilesets.Values), rectangle, viewportPosition, TileWidth, TileHeight);
            }
        }
    }
    ...
    {
        void Draw(Map result, SpriteBatch batch, IList<Tileset> tilesets, Rectangle rectangle, Vector2 viewportPosition, int tileWidth, int tileHeight);
    }
}
